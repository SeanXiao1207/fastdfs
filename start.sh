...
function fdfs_set () {
    if [ "$1" = "monitor" ] ; then
        if [ -n "$TRACKER_SERVER" ] ; then  
            arr=$(echo $TRACKER_SERVER|tr "," "\n")
            for addr in $arr; do
                sed -i "/tracker_server=tracker:22122/a\tracker_server=${addr}" /etc/fdfs/client.conf
            done
            sed -i "/tracker_server=tracker:22122/d" /etc/fdfs/client.conf
        fi
    ...
    if [ -n "$TRACKER_SERVER" ] ; then  
        arr=$(echo $TRACKER_SERVER|tr "," "\n")
        echo ${arr} >> /tmp/sean.txt
        for addrr in $arr; do
            echo ${addrr} >> /tmp/sean.txt
            sed -i "/tracker_server=tracker:22122/a\tracker_server=${addrr}" /etc/fdfs/storage.conf
            sed -i "/tracker_server=tracker:22122/a\tracker_server=${addrr}" /etc/fdfs/client.conf
            sed -i "/tracker_server=tracker:22122/a\tracker_server=${addrr}" /etc/fdfs/mod_fastdfs.conf
        done
        sed -i "/tracker_server=tracker:22122/d" /etc/fdfs/storage.conf
        sed -i "/tracker_server=tracker:22122/d" /etc/fdfs/client.conf
        sed -i "/tracker_server=tracker:22122/d" /etc/fdfs/mod_fastdfs.conf
    fi
